...
<script type="text/javascript">
	...
	function setValue(cust_gubun,erp_code,cust_kor_name,cust_address){
		
		$('#erp_code').val(erp_code) ; 
		$('#info2').val(cust_kor_name) ; 
		$('#info3').val(cust_address) ; 
		closeLayer('2') ; 
	}
	...
	function makeCustInfo(data){
		...
		if(resultVO != null && resultVO.length != '0'){
			
			
			for(var i =0 ; i < resultVO.length; i++){
				str += '<tr> ' ;
				str += '	<th scope="row">대표자명</th> ' ;
				str += '	<td>'+common.nvl(resultVO[i].ceo, '')+'</td> ' ;
				str += '</tr> ' ;
				str += '<tr> ' ;
				str += '	<th scope="row">기관명</th> ' ;
				str += '	<td>'+common.nvl(resultVO[i].cust_kor_name, '')+'</td> ' ;
				str += '</tr> ' ;
				str += '<tr> ' ;
				str += '	<th scope="row">주소</th> ' ;
				str += '	<td>'+common.nvl(resultVO[i].cust_address, '')+'</td> ' ;
				str += '</tr> ' ;
				str += '<tr> ' ;
				str += '<td colspan="2"><button type="button" class="btn_ico_save" style="width:360px !important" id="btn1" onclick="javascript:setValue(\''+common.nvl(resultVO[i].cust_gubun, '')+'\',\''+common.nvl(resultVO[i].erp_code, '')+'\',\''+common.nvl(resultVO[i].cust_kor_name , '')+'\',\''+common.nvl(resultVO[i].cust_address , '')+'\',);"><span>확인</span></button></td>';
				str += '</tr> ' ;
				$('#tabTbody').append(str) ;
	...
	function goSave(){
		...
		if(common.isEmpty($('#dept1_nm').val())){
			alert("근무부서를 입력해 주세요.") ; 
			return ; 
		}
		
		if(common.isEmpty($('#dept_grade_nm').val())){
			alert("직책을 입력해 주세요.") ; 
			return ; 
		}
	...
	function registResult(data){
		...
		alert(msg) ; 
		if(returnCode == "000") location.href='/fr/login/form.do' ; 
	}
...
<form name="procFrm" id="procFrm" method="post" onsubmit="return false;">
	...
	<input type="hidden" name="pageType" id="pageType" value="join"/>
	...
	<table class="vType_line mgb20">
		...
		<tbody>
			<tr>
				...
				<td colspan="3">
					<select id="emp_grade" name="emp_grade" title="회원유형 선택" class="w260">
						<option value="C001">고객사 대표</option>
					</select>
				</td>
			</tr>
			<tr>
				...
				<td colspan="3">
					<input type="text" id="info2" title="소속기관 명 입력" class="w260 mgr5" readonly="readonly" />
			...
			<tr>
				...
				<td>
					<input type="text" id="dept1_nm" name="dept1_nm" title="근무부서 입력" maxlength="20"/>
				</td>
				<th scope="row">직책<span class="request">필수입력</span></th>
				<td>
					<input type="text" id="dept_grade_nm" name="dept_grade_nm" title="직책 입력" maxlength="20" />
				</td>
			</tr>
			<tr>
	...
	<div class="layer_contents">
		...
		<table class="vType_line mgb20">
			...
			<tr>
				<th scope="row">선택</th>
				<td>
					<input type="radio" name="search_type1" class="mgr5" value="1" checked>기관명
				</td>
			</tr>
			<tr>
				<th scope="row">검색</th>
				<td>
					<input type="text" id="search_text" name="search_text" title="기관명"  />
				</td>
			</tr>
		</table>
		<div class="btn_wrap">
