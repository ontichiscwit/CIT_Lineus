...
<script type="text/javascript">
	$(document).ready(function(){
		...
		commonCode.getCodeList('COMMON' , 'CD06' , 'phone1') ; 				/**	핸드폰번호	*/
		...
		$('#cp_phone1').val(common.spritStr('${ frUserInfo.company_no}' , 1, '-')) ; 
		$('#cp_phone2').val(common.spritStr('${ frUserInfo.company_no}' , 2, '-')) ; 
		$('#cp_phone3').val(common.spritStr('${ frUserInfo.company_no}' , 3, '-')) ;
		
		var datas = {'erp_code' 				: '${ frUserInfo.erp_code }' , 'is_page_gbn' : 'fr'} ; 
		common.ajaxCall(datas , '/ad/member/getCustInfo2.do', 'makeCustInfo') ;
		
		if( "C001" == '${ frUserInfo.cust_gubun }' || "C004" == '${ frUserInfo.cust_gubun }' ){
			
			$('#emp_name').attr("readonly","readonly");
			$('#dept1_nm').attr("readonly","readonly");
			$('#dept_grade_nm').attr("readonly","readonly");
			...
			$('#cp_phone1').attr("readonly","readonly");
			$('#cp_phone2').attr("readonly","readonly");
			$('#cp_phone3').attr("readonly","readonly");
		}
	...
	function initView(){ 
		var datas = {'emp_id' : '${ frUserInfo.emp_id }' , 'is_page_gbn' : 'fr'}
	...
	<div id="jw_contents">
		...
		<table class="sType mgb30">
			...
			<tr>
			    ...
			    <td><input type="password" class="w160" name="changePass" id="changePass"></td>
			    <!-- <td colspan="4" class="colorRed">※ 변경하실 비밀번호를 입력해주세요.<br>(영문자, 숫자, 특수문자 또는 이의 조합 4자리 이상 12자리 이하)</td> -->
				<td colspan="6" class="colorRed">※ 변경하실 비밀번호를 입력해주세요. (대소문자 구분 + 숫자 + 특수문자 포함 8자리 이상 20자리 이하)</td>			    
			</tr>
			<tr>
			...
			<tr>
			    <th scope="row" colspan="2">이름<span class="request">필수입력</span></th>
			    <td colspan="7"><input type="text" id="emp_name" name="emp_name" value="${ frUserInfo.emp_name }"></td>
			</tr>
			<tr>
			    <th scope="row" colspan="2">근무부서</th>
			    <td><input type="text" id="dept1_nm" name="dept1_nm" value="${ frUserInfo.dept1_nm }"></td>
			    <th scope="row" colspan="2">직책</th>
			    <td colspan="2"><input type="text" id="dept_grade_nm" name="dept_grade_nm" value="${ frUserInfo.dept_grade_nm }"></td>
			</tr>
			<tr>
			    <th scope="row" colspan="2">이메일<span class="request">필수입력</span></th>
			    <td colspan="6"><input type="text" class="w150 mgr5" id="email1">@ <input type="text" id="email2" class="w250 mgl5"></td>
			</tr>
			<tr>
			    <th scope="row" colspan="2">연락처(회사)<span class="request">필수입력</span></th>
			    <td colspan="6">
			        <input type="text" class="w115 mgr5" id="cp_phone1"/>
			        <input type="text" class="w150 mgr5" id="cp_phone2">
			        <input type="text" class="w150" id="cp_phone3">
			    </td>
			</tr>
			<tr>
			    <th scope="row" colspan="2">연락처(핸드폰)<span class="request">필수입력</span></th>
			    <td colspan="6">
		...
		<table class="sType mgb30">
			...
			<tr>
			    <th>고객사명</th>
			    <td colspan="3"><input type="text" readonly="readonly" id="cust_kor_name"></td>
