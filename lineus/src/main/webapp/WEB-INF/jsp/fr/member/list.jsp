...
<script type="text/javascript">
	$(document).ready(function(){
		...
		commonCode.getCodeList('COMMON' , 'CD01' , 'search_type2') ; 		/**	회원 등급		*/
		commonCode.getCodeList('COMMON' , 'CD05' , 'search_type3') ; 		/**	AS결재자		 */
		
		goList(1) ; 
		<c:if test="${ frUserInfo.cust_gubun eq 'C001'}">
		 $(".btn_wrap").hide();
		</c:if>
		<c:if test="${ frUserInfo.cust_gubun eq 'C004'}">
		 $(".btn_wrap").hide();
		</c:if>
	}) ; 
		...
		<table class="hType_line mgb10">
			...
			<tbody>
				<tr>
					...
					<td>
						<select name ="search_type1" id="search_type1"></select>
					</td>
					<th scope="row" >계정유형</th>
					<td>
						<select name ="search_type2" id="search_type2"></select>
					</td>
					<th scope="row">결재 승인 권한</th>
					<td>
						<select name ="search_type3" id="search_type3"></select>
					</td>
				</tr>
				<tr>	
					<th scope="row">통합검색</th>
					<td colspan ="5" >
						<input type="text" style="width:770px; float:left; margin-right:5px;"  placeholder="이름 /아이디 /근무부서" name ="search_text" id="search_text"/>
						<button style="float:left;" class="btn_ico_search dblue small" onclick="javascript:goList(1);"><span>검색</span></button>
					</td>
				</tr>
			</tbody>
		</table>
