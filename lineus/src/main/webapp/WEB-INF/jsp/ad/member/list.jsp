...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#search_text").keyup(function(e){if(e.keyCode == 13)  goList(1); });			//enter키 이벤트
	...
	function getMemberList(data){
		...
		if(resultList != null && resultList.length > 0){
			...
			for(var i = 0 ; i < resultList.length ; i++){
				...
				str += '	<td onclick=\'event.cancelBubble=true;\'><input type="checkbox" name="chk" value="'+common.nvl(datas.emp_id , '')+'"/></td> ' ;
...
<table class="sType mgb10">
	...
	<tr>
		...
		<td>
			<input type="text" class="w640 mgr5" id="search_text" name="search_text" title="통합 검색 키워드 입력" placeholder="회원 아이디 /회원 이름 /거래처이름 /부서명 /팀명 /전화번호 /이메일 /계정상태">
