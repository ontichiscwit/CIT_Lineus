...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		commonCode.getCodeList('CUST' , 'CD04' , 'search_type3') ; 		/**	A/S 결재 프로세스 여부	*/
	...
	function getCustList(data) {
		...
		if(resultList != null && resultList.length > 0){
			for(var i = 0 ; i < resultList.length ; i++){
				...
				str += "	<td>" + common.nvl(datas.buss_item, '-') + "</td>" ;
				str += "	<td>" + common.nvl(datas.buss_condition, '-') + "</td>" ;
				str += "	<td>" + common.nvl(datas.cust_address, '-') + "</td>" ;
				str += "	<td>" + common.nvl(datas.charger_nm, '-')+ "</td>" ;
				str += "	<td>" + common.nvl(datas.as_approval_yn_nm, '-') + "</td>" ;
				str += "</tr>" ;
			...
			commonTable.notData(9 , '조회된 데이터가 없습니다.' , 'custList') ; 
			$("#pagination").html('');
		} 
	}
...
<table class="sType mgb10">
	...
	<tr>
		...
		<td>
			<select id="search_type2" name="search_type2" title="거래상태"></select>
		</td>
		<th scope="row">A/S 결재 프로세스</th>
		<td>
			<select id="search_type3" name="search_type3" title="A/S 결재 프로세스 선택"></select>
		</td>
	</tr>
	<tr>
		<th scope="row">통합검색</th>
		<td colspan="5">
			<input type="text" placeholder="거래처명  /거래처코드  /대표관리자명" id="search_text" name="search_text" class="w640 mgr5" title="통합 검색 키워드 입력" value="${ vo.search_text }" /><button type="button" class="btn_ico_reset mgr5" onclick="searchReset();"><span>초기화</span></button><button type="button" class="btn_ico_search" onclick="custList(1);"><span>검색</span></button>
		</td>
	</tr>
</table>
...
<table class="hType mgb10">
	...
	<thead>
		<tr>
			...
			<th scope="col">업태</th>
			<th scope="col">주소</th>
			<th scope="col">담당자명(CIT)</th>
			<th scope="col">A/S결재</th>
