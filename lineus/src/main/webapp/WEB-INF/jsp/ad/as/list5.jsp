...
<script type="text/javascript">
	...
	function initForm(){
		...
		$('#search_type20').val('${ vo.search_type20 }'); 
		...
		$('#search_type20').val('${ vo.search_type20 }');     // 테스트 조회 화면
		$('#page').val('${ vo.page}') ;
		...
		$("#search_type20").keyup(function(e){if(e.keyCode == 13)  getAsList(1); }); //테스트 조회 화면
	...
	function getAsList(pageIndex) {					//페이지이동-이벤트사용
		...
		f.target = '' ; 							//폼의 타겟 지정 
		f.action = '/ad/as/list5.do' ; 				//이동할페이지 		／／테스트 조회 화면	
		f.submit() ; 
	}
	...
	function setAsList(data) {
		...
		if (resultList != null && resultList.length > 0) {
			...
			for(var i = 0 ; i < resultList.length ; i++){
				...
				if(common.nvl(datas.appr_yn1, '') == 'Y'){
					str += '<td>결재</td> ' ;
				} else if(common.nvl(datas.appr_yn1, '') == 'N'){
					str += '<td>미결</td> ' ;
				} else if(common.nvl(datas.appr_yn1, '') == 'R'){
					str += '<td>부결</td> ' ;
				} else {
					str += '<td>-</td> ' ;
				}
...
<table class="sType mgb10">
	...
	<tbody id="asSearchTbody">
		...
		<tr>
			...
			<td> 
				<input type="text" placeholder="JW그룹웨어 문서ID/IDX" name="search_type20" id="search_type20" title="JW그룹웨어 문서ID" > 
			</td>
		</tr>
		...
		<tr> 
			...
			<td colspan="5"> 
				<input type="text" placeholder="접수번호  /거래처명  /거래처코드  /처리담당자명  /요청자명  /접수내용 /처리내용" name="search_text" id="search_text" class="w630 mgr10"  style="ime-mode:active;"  title="통합 검색 키워드 입력" />
