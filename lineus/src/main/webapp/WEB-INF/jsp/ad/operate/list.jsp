...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		commonCode.getCodeList('OPERATE' , 'OP01' , 'search_type6') ; 		/**	상태		*/
		...
		$( "#search_type7" ).val($.datepicker.formatDate('yy/mm/dd', new Date())).datepicker(datepicker);
		$( "#search_type8" ).val($.datepicker.formatDate('yy/mm/dd', new Date())).datepicker(datepicker);
	...
	function setOperateList(data) {
		...
		if(resultList != null && resultList.length > 0){
			for(var i = 0 ; i < resultList.length ; i++){
				...
				str += "	<td onclick='event.cancelBubble=true;'><input type=\"checkbox\" id=\"chkOperNo\" name=\"chk\" class=\"\" value='" + datas.oper_seq +"'/></td>" ;
				...
				str += "	<td>" + common.nvl(datas.system_nm, '-') + "</td>" ;
			...
			$('#count').html('0') ; 
			commonTable.notData(15 , '조회된 데이터가 없습니다.' , 'getOperateList') ; 
			$("#pagination").html('');
		} 
	}
	...
	function delOpProc() {
		...
		$("input[name=chk]:checked").each(function() {
			chkVal = $(this).val() ;
			op_seq = chkVal.split('@')[0];
			
			if (del_dtl_seq == '') del_dtl_seq = op_seq;
			else del_dtl_seq = del_dtl_seq + "@"+ op_seq;
			
		});
		
		if (common.nvl(del_dtl_seq,'') != '') {
			var datas = { "del_dtl_seq": del_dtl_seq };
			common.ajaxCall(datas , '/ad/operate/delProc.do', 'delProcReturn') ;
		}
	}
...
<div class="tit_wrap">
	<h2 class="tit_ico_customer">운영시스템 관리<span class="tit_depth no_arrow mgl10 mgt8">검색 조건을 선택해 주세요.</span></h2>
	<div class="location">
		<a href="/ad/main/list.do" class="home">Home</a>
		<a href="/ad/operate/list.do" class="depth"><span class="here">운영시스템 관리</span></a>
	</div>
</div>
<!-- search -->
...
<table class="sType mgb10">
	...
	<tr>
		...
		<td>
			<input id="search_type5" type="text" name="search_type5" title="운영담당자" class="w155"/>
		</td>
		<th scope="row">운영상태</th>
		<td colspan="3">
			<select id="search_type6" name="search_type6" title="운영상태" class="w155"></select>
		</td>
	</tr>
	<tr>
		<th scope="row">통합검색</th>
		<td colspan="5">
			<input type="text" id="search_text" placeholder="시스템명 /프로젝트명 /거래처명/서버(내부)IP /서버(외부)IP" name="search_text" class="w640 mgr10" title="통합 검색 키워드 입력" value="${ vo.search_text }" /><button type="button" class="btn_ico_reset mgr5" onclick="searchReset();"><span>초기화</span></button><button type="button" class="btn_ico_search" onclick="getOperateList(1);"><span>검색</span></button>
		</td>
	</tr>
</table>
...
<div class="info_upper mgb5">
	...
	<div class="floatR">
		<button type="button" class="btn_ico_circle_plus" onclick="operateRegist();"><span>운영시스템 등록</span></button>
