...
public class MenuAuthFilter implements Filter{
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		try{
			...
			if(pathUri.startsWith("/ad") && frUserInfo == null && !pathUri.contains("confirm/view.do")) {
				...
				if(!(pathUri.startsWith("/fr/login") || pathUri.startsWith("/fr/join") || pathUri.startsWith("/fr/agreement"))){
...
//				}
	...
	private boolean isAccept(String pathUri, List<String> acceptUrlList) {
		...
		for (String acceptUrl : acceptUrlList){
			...
			if (acceptUrl.equals(pathUri)){
				logger.debug("user request url has Auth");
				return true;
			}
		}
