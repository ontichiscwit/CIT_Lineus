...
public class SsStringUtil {
	...
	*/
	...
	/**
	 * 로그인 비밀번호 정규식 체크  (비밀번호 체계 : 대소문자 구분 + 숫자 + 특수문자 포함 8자리 이상 20자리이하)  2020.09.10.
	 * @param pw
	 * @return 
	 */	 
	public static boolean validationPass(String pw) {  
		//Pattern p = Pattern.compile("^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[$@$!%*#?&])[A-Za-z[0-9]$@$!%*#?&]{8,}$");  		
		//Pattern p = Pattern.compile("^(?=.*[A-Za-z])(?=.*[0-9])(?=.*[$@$!%*#.}{_)(?&])[A-Za-z[0-9]$@$!%*#.}{_)(?&]{8,20}$"); //영문 + 숫자 + 특수문자 조합 8자리이상 20자리이하 	
		Pattern p = Pattern.compile("^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[$@$!%*#.}{_)(?&])[A-Za-z[0-9]$@$!%*#.}{_)(?&]{8,20}$"); //대문자 + 소문자 + 숫자 + 특수문자 조합 8자리이상 20자리이하	
