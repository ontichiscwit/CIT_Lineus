...
public class FrMemberController {
	...
	public void registMember(@ModelAttribute("vo") UserVO vo, HttpServletRequest request, HttpServletResponse response , HttpSession session) throws Exception {
		...
		vo.setErp_code(userInfo.getCust_code());
		...
		else if("passChange".equals(pageType)) returnValue = memberService.registMemberPassChange(vo) ;  	/**	비번변경	*/
		else if("changeIndividual".equals(pageType)) returnValue = memberService.registMemberPUpdate(vo) ;  	/**	비번변경	*/
