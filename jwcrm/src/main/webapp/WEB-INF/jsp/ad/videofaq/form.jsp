...
<script type="text/javascript">
			...
			success : function(data){
				...
				if(resultVO != null){
					...
					if (info != null){
						...
						if (reg_id == group_userid || emp_grade == 'C001' || emp_grade == 'C002') {
					...
					} else {																			//2021.04.02.이설아 HIS운영지원팀 요청으로 내부관리자도 게시글의 수정권한을 갖도록 소스수정
						if (reg_id == group_userid ||  emp_grade == 'C001' || emp_grade == 'C002') {	//C001:마스터관리자 ,C002:내부관리자 
							addUpFile();
						} else {
							$('#fileWrap').html('-');
						}
	...
	function goSubmit() {
		...
		if (confirm("작성된 정보로 FAQ(동영상)을 등록하시겠습니까?")) {	
...
<div class="tit_wrap">
	...
	<h2 class="tit_ico_notice">FAQ(동영상)관리</h2>
	
	<div class="location">
		<a href="/ad/main/list.do" class="home">Home</a>
		<a href="#" class="depth">게시판관리</a>
		<a href="/ad/videofaq/list.do" class="depth">
		<span class="here">FAQ(동영상)관리</span>
		</a>
	</div>
...
<table class="sType mgb10">
	<caption>FAQ(동영상) 상세정보 등록</caption>
	<colgroup>
	...
	<tr>
		...
		<td>
			<select id="work_type" name="work_type" class="w200" title="시스템유형 선택" ></select>
		</td>
	</tr>
	<tr>
		<th scope="row">제목</th>
