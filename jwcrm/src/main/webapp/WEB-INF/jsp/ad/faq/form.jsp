...
<script type="text/javascript">
	...
	function initView() {
			...
			success : function(data){
				...
				if(resultVO != null){
					...
					if (info != null){
						...
						//2024.05.03_김규민 나만보기 버튼 기능 추가
						$(":input:radio[name=open_type]:input[value="+info.open_type+"]").attr("checked",true).click();
						
						//2024.09.24_김규민 BEST 상담사례 버튼 기능 추가
						$(":input:radio[name=best_type]:input[value="+info.best_type+"]").attr("checked",true).click();
						
						//2024.05.07_김규민 나만보기 버튼은 작성자만 볼 수 있도록 설정
						if (reg_id !== group_userid){
							$('#onlyme_open_yn').hide();
						}
						
						//2024.09.24_김규민 노출구분이 '나만보기'일 경우 BEST 상담사례 구분은 '일반공지' 값 + BEST 상담사례 행은 숨기기
						if (info.open_type == 'C002'){
							$('#best_faq').val('N');
							$('#best_faq').hide();
						}
					}
		...
		$('[id^=btnTab]').bind("click",function(){
			...
			else if(dataid=='subTab3') setLikeData();
	...
	function setDownHistData(obj){
 	...
 	function setAnswerData(){
		...
		common.ajaxCall(datas , '/ad/board/getFaqAwsList.do', 'getFaqAnswer') ;
	}
	...
	function setLikeData(){
		var datas = {'faq_seq' : '${ vo.seq }' } ;
		common.ajaxCall(datas , '/ad/board/getFaqLikeList.do', 'getFaqLike') ;
	}
	...
	function getFaqLike(data){
		...
		if(resultVO != null && resultVO.lenght !=0){
			...
			for(var i=0; i < resultVO.length; i++){
			  LikeStr += "<tr>";
			  LikeStr += "	<td>"+ (i+1) +"</td>";
			  LikeStr += "	<td>"+resultVO[i].crm_code+"</td>";
			  LikeStr += "	<td>"+resultVO[i].w_cust_nm+"</td>";
			  LikeStr += "	<td>"+resultVO[i].w_nm+"</td>";
			  LikeStr += "	<td>"+resultVO[i].w_tel_no+"</td>";
			  LikeStr += "	<td>"+resultVO[i].w_date+"</td>";
			  LikeStr += "</tr>";
			}
	...
	function aswForm(){
		...
		common.ajaxCall(datas, '/ad/board/faqawsform.do', 'registResult') ;
	}
	...
	function deleteAnswer(data){
		...
		var datas = {'seq' : data}
		common.ajaxCall(datas, '/ad/board/faqawsDel.do', 'registResult') ;
	}
	...
	function goSubmit() {
	    ...
	    var replacedContent = content.replace(/상담사례\((\d+)\)/g, function(match, number) {
	    	...
	    	if($('#faqcount').val() == 0){
	    		...
	    		if($("input[name='open_type']:checked").val() == 'C001'){
					...
					common.ajaxCall(datas, '/ad/board/getFaqonlymeexist.do','setFaqonlymeexist'); //상담사례 번호 나만보기 유형 검토
		    		
		    		if($('#faqonlymecount').val() == 0){
		    			alert("상담사례(" + number + ")을 수정해주세요.");
		    		}
	    		}	
	    	}
	    ...
	    if($('#faqcount').val() == 0 || $('#faqallcount').val() == 0 || $('#faqonlymecount').val() == 0){
			$('#content').focus();
			return;
	    }
		...
		if (confirm("작성된 정보로 상담사례를 등록하시겠습니까?")) {	
	...
	function setFaqonlymeexist(data){
		var result = data.resultList != "undefined" ? data.resultList : null;
		if(data.resultList[0].CNT == 0){
		alert("아래 상담사례 번호는 다른 직원 상담사례의 '나만보기' 상태입니다.");
		$('#faqonlymecount').val(data.resultList[0].CNT);
		return;
		}
	}
...
<input type="hidden" name="pageGubun" id="pageGubun" value="faq" />
...
<input type="hidden" name="faqonlymecount" id="faqonlymecount" value="1" />
...
<div class="tit_wrap">
	...
	<h2 class="tit_ico_faq">상담사례 관리</h2>
	
	<div class="location">
		<a href="/ad/main/list.do" class="home">Home</a>
		<a href="#" class="depth">게시판관리</a>
		<a href="/ad/faq/list.do" class="depth">
		<span class="here">상담사례 관리</span>
		</a>
	</div>
...
<table class="sType mgb10">
	...
	<tr id = "onlyme_open_yn">
		<th scope="row" >노출구분</th>
		<td>
			<input type="radio" name="open_type" value="C001" class="mgr5" onclick="$('#best_faq').show();" checked>전체공지<span class="mgr20"></span>
			<input type="radio" name="open_type" value="C002" class="mgr5" onclick="$('#best_faq').hide();">나만보기<span class="mgr20"></span>
		</td>
	</tr>
	<tr id = "best_faq">
		<th scope="row" >베스트구분</th>
		<td>
			<input type="radio" name="best_type" value="N" class="mgr5" checked>일반공지<span class="mgr20"></span>
			<input type="radio" name="best_type" value="Y" class="mgr5" >BEST<span class="mgr20"></span>
		</td>
	</tr>
	<tr>
		<th scope="row">제목</th>
...
<ul class="tab_line list2 mgb20">
	...
	<li><a href="#" id="btnTab3" data-id="subTab3">좋아요 내역</a></li>
...
<div id="subTab3" style="display:none;max-height:300px;overflow-y:auto;margin-bottom:20px;">
	<table class="hType">
		...
		<thead>
			<tr>
				<th scope="col">NO</th>
				<th scope="col">CRM코드</th>
				<th scope="col">거래처명</th>
				<th scope="col">사용자명</th>
				<th scope="col">사용자 연락처</th>
				<th scope="col">좋아요 표시일시</th>
			</tr>
		</thead>
		<tbody id="LikeList"></tbody>
	</table>
</div>
